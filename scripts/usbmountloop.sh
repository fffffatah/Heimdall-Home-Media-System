...
while true; do
 ...
 TAKENSTORAGENUM='0'
 NEWFOLDER=''
 MOUNTEDLIST=$(cd /home/ubuntu/system_a/Heimdall-Home-Media-System/scripts/drives && ls | sed 's/,.*//')
  ...
  if [[ $CONNECTEDUSB ]]; then
   ...
   do
    if [[ "$devlist" != "$mountlist" ]]; then
     GETFILENAME=$(cd /home/ubuntu/system_a/Heimdall-Home-Media-System/scripts/drives && grep "^$mountlist")
     UNMOUNTABLE=$(echo $GETFILENAME  | sed 's:.*,::')
    fi
   done < <(cd /home/ubuntu/system_a/Heimdall-Home-Media-System/scripts/drives && ls | sed 's/,.*//')
